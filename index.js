...
vConsolePlugin.prototype.apply = function (compiler) {
  ...
  const pathVconsole = path.resolve(
    __dirname,
    disableInPC ? './src/disable-in-pc.js' : './src/vconsole.js'
  )
    ...
    if (enable) {
      if (typeof entry === 'string') {
        if (!checkFilter([entry], that.options.filter)) {
          ...
          console.warn(
            '[vconsole-webpack-plugin] 暂不支持 entry 为 string 类型的情况\n'
          )
        }
        ...
        for (let key in entry) {
          if (that.options.filter.indexOf(key) < 0) {
            if (
              Object.prototype.toString.call(entry[key]) === '[object Array]'
            ) {
